...
- **引擎**: sisyphus-api-engine (独立 PyPI 包,YAML 驱动)
...
引擎已作为独立 PyPI 包发布,通过 `backend/pyproject.toml` 依赖自动安装。

```bash
# 安装/更新引擎
uv pip install sisyphus-api-engine          # 安装最新版
uv pip install --upgrade sisyphus-api-engine # 更新到最新版

# 执行测试用例 (CLI)
sisyphus-api-engine --case <yaml_file_path>                    # 默认文本报告
sisyphus-api-engine --case <yaml_file_path> -O json            # JSON 输出 (平台集成)
sisyphus-api-engine --case <yaml_file_path> -O allure          # Allure 报告
sisyphus-api-engine --case <yaml_file_path> -O html            # HTML 报告
```
...
> 引擎已发布为独立 PyPI 包 (`pip install sisyphus-api-engine`),后端通过 `pyproject.toml` 声明依赖,`uv sync` 自动安装。
...
2. **executor_adapter.py**: 调用引擎 CLI (`sisyphus-api-engine --case <yaml> -O json`)
...
1. **引擎未安装**: `uv pip install sisyphus-api-engine`
