...
async def get_dashboard_stats(
    ...
    active_tasks_stmt = select(func.count()).select_from(TestReport).where(
        TestReport.created_at >= seven_days_ago
    )
    active_tasks = (await session.execute(active_tasks_stmt)).scalar() or 0
    
    # 计算测试覆盖率 (成功率)
    success_stmt = select(func.count()).select_from(TestReport).where(
