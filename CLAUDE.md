...
- **引擎**: 内嵌于 `backend/app/engine/` (原 sisyphus-api-engine,YAML 驱动)
...
│                    Backend (FastAPI)                        │
│  ┌─────────────┐  ┌──────────────┐  ┌──────────────────┐  │
│  │ API Routes  │→ │   Services   │→ │  app.engine       │  │
│  │ (endpoints) │  │ (业务逻辑层)  │  │ (内嵌,YAML驱动)   │  │
│  └─────────────┘  └──────────────┘  └──────────────────┘  │
│         │                                                  │
│         ▼                                                  │
│  ┌─────────────┐  ┌──────────────┐  ┌──────────────────┐  │
...
# 依赖管理
uv sync                                    # 同步依赖 (引擎已内嵌于 app/engine/)
...
引擎源码已内嵌于 `backend/app/engine/`,通过 Python API 直接调用:
...
> 引擎源码位于 `backend/app/engine/`,通过 `from app.engine.core.runner import load_case, run_case` 直接调用。
...
1. **engine_executor.py**: 封装 `EngineExecutor` 类,调用内嵌引擎 `load_case()` + `run_case()`
2. **execution/executor_adapter.py**: 异步适配器,通过 `asyncio.to_thread()` 包装同步引擎调用
3. **execution/yaml_generator.py**: 前端参数 → YAML 文件 (保存至 temp/)
4. **execution/result_parser.py**: 解析 JSON 响应 → 数据库记录
...
├── engine/                      # 内嵌测试执行引擎 (原 sisyphus-api-engine)
│   ├── core/runner.py           # load_case() / run_case()
│   ├── core/models.py           # CaseModel (输入)
│   └── result/models.py         # ExecutionResult (输出)
├── models/                      # SQLAlchemy 模型 (必须全部导入到 db.py)
├── schemas/                     # Pydantic Schema
├── api/v1/endpoints/            # API 路由端点
├── services/
│   ├── engine_executor.py       # EngineExecutor 封装
│   └── execution/               # 测试执行服务
...
1. **导入错误**: 确保 `from app.engine.core.runner import run_case` 可导入 (运行 `uv sync`)
2. **YAML 语法错误**: 参考 `docs/api-engine/Sisyphus-api-engine YAML 输入规范.md`
...
- [YAML 输入规范](./docs/api-engine/Sisyphus-api-engine%20YAML%20输入规范.md)
- [JSON 输出规范](./docs/api-engine/Sisyphus-api-engine%20JSON%20输出规范.md)
- [引擎文档](./docs/api-engine/)
- [变更日志](./CHANGELOG.md)
