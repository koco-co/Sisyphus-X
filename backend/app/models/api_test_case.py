...
class ApiTestExecution(SQLModel, table=True):
    ...
    error_type: Optional[str] = None  # AssertionError, TimeoutError, ConnectionError, etc.
    error_category: Optional[str] = None  # assertion, network, timeout, parsing, business, system
...
class ApiTestStepResult(SQLModel, table=True):
    ...
    # 错误信息
    error_info: Optional[Dict] = Field(default=None, sa_column=Column(JSON))
