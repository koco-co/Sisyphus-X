...
class StreamingResponseGenerator:
    ...
    async def generate(self):
        """生成流式响应"""
        try:
            async for chunk in self.graph.astream_chat(self.requirement_id, self.user_input):
                # 将chunk转换为SSE格式
...
async def clarify_requirement(
    ...
    # 创建流式响应生成器
    generator = StreamingResponseGenerator(graph, request.requirement_id, request.user_input)
...
async def get_conversation(
    ...
    # 目前返回空历史
    return {"requirement_id": requirement_id, "messages": [], "state": None}
...
async def reset_clarification(
    ...
    """
    # TODO: 实现重置逻辑
    return {"requirement_id": requirement_id, "status": "reset", "message": "对话已重置"}
