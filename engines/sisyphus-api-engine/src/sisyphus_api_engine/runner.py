"""场景执行器 - 解析 YAML、按序执行步骤、生成 JSON 输出（EG-026/027）"""
...
from sisyphus_api_engine.executors.request import execute_request_step
from sisyphus_api_engine.models import CaseModel, Config, StepDefinition


def load_case(yaml_path: str | Path) -> CaseModel:
    """加载并校验 YAML 用例（EG-004）。"""
    path = Path(yaml_path)
    if not path.exists():
...
def run_case(case: CaseModel) -> dict[str, Any]:
    """
    执行用例，返回符合 JSON 输出规范的顶层结构（EG-026/027）。
    """
