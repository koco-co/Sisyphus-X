...
async def list_plans(
    ...
    return PageResponse(items=plans, total=total, page=page, size=size, pages=pages)
...
async def update_plan(plan_id: int, data: PlanUpdate, session: AsyncSession = Depends(get_session)):
    """更新测试计划"""
    plan = await session.get(TestPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
...
async def pause_plan(plan_id: int, session: AsyncSession = Depends(get_session)):
    """暂停测试计划"""
    plan = await session.get(TestPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
...
async def resume_plan(plan_id: int, session: AsyncSession = Depends(get_session)):
    """恢复测试计划"""
    plan = await session.get(TestPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
...
async def trigger_plan(plan_id: int, session: AsyncSession = Depends(get_session)):
    """手动触发测试计划执行"""
    plan = await session.get(TestPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
